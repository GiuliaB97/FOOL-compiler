...
public class CodeGenerationASTVisitor extends BaseASTVisitor<String, VoidException> {
	private static final List<List<String>> dispatchTables = new ArrayList<>();	/*EX OO: Le Dispatch Table di tutte le classi vengono create staticamente dal compilatore in campo privato dispatchTables del visitor
	...
	public String visitNode(FunNode n) {
		...
		for (Node dec : n.declist) {
			...
			if( ((DecNode)dec).getType() instanceof ArrowTypeNode) {//se tra i dichiarazioni  vi siano ID di tipo funzionale
				popDecl = nlJoin(popDecl,"pop");//si devono deallocare due cose dallo stack (con due "pop"); questa è la seconda
			}
		}

		for (int i=0;i<n.parlist.size();i++) {
			popParl = nlJoin(popParl,"pop");
			if(n.parlist.get(i).getType() instanceof ArrowTypeNode) {//stessa solfa di cui sopra per i parametri
				popParl = nlJoin(popParl,"pop");
			}
		}
	...
	public String visitNode(ClassCallNode n) {
		...
		List<String> dispatchTable;		/*creo la sua dispatch table 
											per ogni classe; tipo di variabile richiesto da specifica
		 */
